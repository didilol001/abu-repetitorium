...
export default function FlipCard({ question, answer, questionData, onResult, inputMode = false }) {
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 gap-3">
      <div
        className={`perspective flex-1 min-h-0 ${!inputMode ? 'cursor-pointer' : ''}`}
        onClick={handleFlip}
      >
        <div className={`flip-card-inner ${isFlipped ? 'flipped' : ''}`}>
          <div className="flip-card-front bg-abu-card border-2 border-abu-border">
            ...
            {inputMode && (
              <textarea
                className="mt-4 w-full rounded-xl bg-slate-700 border border-slate-500 text-abu-text text-sm p-3 resize-none focus:outline-none focus:border-abu-primary"
                rows={4}
          ...
          <div className="flip-card-back bg-slate-700 border-2 border-slate-500">
            {inputMode && (
              <>
                ...
                <p className="text-abu-text text-sm leading-relaxed whitespace-pre-wrap bg-slate-600 rounded-lg p-2 mb-3">
                  {userAnswer || <span className="text-abu-muted italic">Keine Eingabe</span>}
                </p>
                <div className="border-t border-slate-500 mb-3" />
              </>
            )}
            <p className="text-xs font-semibold text-abu-muted uppercase tracking-wider mb-3">
              Antwort
      ...
      {inputMode && !isFlipped ? (
        <button
          className="flex-shrink-0 w-full py-4 rounded-2xl bg-abu-primary text-white font-bold text-base active:scale-[0.97] transition-transform shadow-lg shadow-blue-500/20"
